...
TOPIC_NAME_RECIVE = 'posts'
TOPIC_NAME_SEND = 'spam-posts'
...
class Post:
    def __init__(self, postId, content, label):
        self.postId = postId
        self.content = content
        self.label = label

    def __repr__(self):
        return f"Comment(commentId={self.postId}, content={self.content}, label={self.label})"
...
def produce(post):
    ...
    preprocessed = preprocess_text(post.content)
    # Vector hóa nội dung
    ...
    producer.send(TOPIC_NAME_SEND, post_dict)
    producer.flush()
    print("Processed post sent:", post_dict)
